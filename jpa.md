...
### 영속성 컨텍스트 특징
```
- 엔티티를 식별자값(@Id, 기본기와 매핑한 값)으로 구분 (영속성 컨텍스트 내부에 `Map` 있음- 키: @Id, 값: 엔티티 인스턴스)
- 영속상태는 식별자값이 반드시 있어야 함.
- 식별자 값이 없으면 예외 발생
```
- 트랜잭션 커밋하는 순간 영속성 컨텍스트에 새로 젖장된 엔티티를 DB에 반영
- 영속성 컨텍스트의 Entity관리 장점
	- 1차 캐시
	- 동일성 보장
	- 트랜잭션 지원하는 쓰기 지연
	- 변경 감지
...
### `엔티티조회`
- `em.find()호출` -> `1차 캐쉬에서 엔티티 조회`-> `없으면 DB에서 조회해서 엔티티 생성` -> `1차 캐시에 저장후에 영속상태 엔티티 반환`

### 수정 순서
1. 트랜잭션 커밋 -> 엔티티 매니저 내부에서 먼저 플러시 호출
2. 엔티티와 스냅샷을 비교해서 변경된 엔티티 찾음
3. 변경된 엔티티가 있으면 수정 쿼리를 생성해서 **쓰기지연 SQL저장소**로 보냄
