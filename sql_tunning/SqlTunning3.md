...
실행계획을 수행해보면 ConsistencyRead(LogicalRead)는 281이 나옵니다..
만약 판매월 + 판매구분으로 인덱스가 잡혀있다면 어떻게될까요?
...
이를 개선하기 위한 첫번째 방법으로 in list를 사용하는 것입니다.

in은 union all+equal(=)을 여러개 연결한것과 동일합니다.
즉, 각 판매월을 = 조건으로 각각 조회하고 이 결과를 합친것이기 때문에 인덱스 효과를 볼수 있습니다.

이 쿼리의 결과는 ConsistencyRead가 314가 됩니다.
...
이 쿼리의 ConsistencyRead는 300이 됩니다.
...
회사는 반드시 입력
지역은 입력하지 않을 수도 있다.
상품명은 단어 중 일부만 입력하고도 조회 가능
인덱스는 회사 + 지역 + 상품명으로 잡혀있습니다.
...
where rnum >= 91
위 쿼리는 앞 페이지는 문제가 없지만, 뒤로 갈수록 성능이 늦어지는 문제를 안고 있습니다.
...
이렇게 인덱스 스캔이 발생한 이유는 union all쿼리의 조건에 있는 도서명 like :book_nm || '%'을 인덱스 조건으로 사용되었기 때문입니다.
...
between은 시작점과 끝점에선 equals(=)와 같은 효과를 발휘합니다.
like는 조건 그 이상을 검색하게 됩니다.
...
쿼리 5 : 2009년 1월과 2월 데이터 모두 읽는다. 1월 데이터를 모두 스캔하는 이유는 '2009'으로 조회 했기 때문.
쿼리 6 : 쿼리 5와 스캔량은 같다. between이라고 하더라도 쿼리 시작값을 '200900'으로 입력하고 조회하면 LIKE와 스캔량이 같음
